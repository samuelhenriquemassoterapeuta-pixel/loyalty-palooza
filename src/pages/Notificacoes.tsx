...
import { ArrowLeft, Bell, CheckCheck, BellOff, Trash2, Loader2 } from "lucide-react";
...
const Notificacoes = () => {
  ...
  const contadorPorTipo = useMemo(() => {
    ...
    notificacoes.forEach((n) => { contador[n.tipo] = (contador[n.tipo] || 0) + 1; });
    return contador;
  }, [notificacoes]);
  ...
  const handleMarcarTodasLidas = async () => {
    ...
    if (error) toast.error("Erro ao marcar notificações");
  ...
  const handleConfirmarExclusao = async () => {
    ...
    if (error) toast.error("Erro ao excluir notificação");
    else toast.success("Notificação excluída");
  };
  ...
  const handleExcluirTodas = async () => {
    ...
    if (error) toast.error("Erro ao excluir notificações");
    else toast.success("Todas as notificações foram excluídas");
  };
        ...
        <div className="max-w-lg lg:max-w-4xl mx-auto px-4 lg:px-8 safe-top pt-4 relative z-10">
          ...
          >
            ...
            <div className="flex items-center gap-1">
              {notificacoes.length > 0 && (
                <Button variant="ghost" size="sm" onClick={() => setConfirmarExcluirTodas(true)} className="text-destructive hover:text-destructive">
                  <Trash2 size={16} className="mr-1" /> Limpar
                </Button>
              )}
              {naoLidas.length > 0 && (
                <Button variant="ghost" size="sm" onClick={handleMarcarTodasLidas} className="text-primary">
                  <CheckCheck size={16} className="mr-1" /> Marcar todas
                </Button>
              )}
            </div>
          </motion.div>
            ...
            {TIPOS_NOTIFICACAO.map((tipo) => {
              ...
              return (
                <Button
                  ...
                  className={`shrink-0 gap-1.5 rounded-xl ${isActive ? "" : "bg-card/80"}`}
                >
                  <span>{tipo.icon}</span>
          ...
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              {filtroAtivo !== "todos" && (
                <Button variant="ghost" size="sm" onClick={() => setFiltroAtivo("todos")} className="mt-2 text-primary">
                  Ver todas
                </Button>
              )}
            </div>
                    ...
                    >
                      ...
                      >
                        <div className="flex gap-3">
                          <div className="text-2xl">{getIconByTipo(notificacao.tipo)}</div>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-start justify-between gap-2">
                              ...
                              <div className="flex items-center gap-1 shrink-0">
                                {!notificacao.lida && <span className="w-2 h-2 bg-primary rounded-full" />}
                                <Button
                                  variant="ghost"
                                  size="icon"
        ...
        <AlertDialog open={confirmarExcluirTodas} onOpenChange={setConfirmarExcluirTodas}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>Todas as {notificacoes.length} notificações serão removidas permanentemente.</AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel disabled={excluindoTodas}>Cancelar</AlertDialogCancel>
              <AlertDialogAction onClick={handleExcluirTodas} disabled={excluindoTodas} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">
                {excluindoTodas ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Excluindo...</> : "Excluir todas"}
              </AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
