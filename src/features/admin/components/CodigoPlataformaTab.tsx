...
CASHBACK EXPIRANDO
  ...
  └→ Debita automaticamente após vencimento`}
      ...
      {/* 11. Monetização de Cursos */}
      <SectionCollapsible
        title="Monetização de Cursos"
        subtitle="Precificação, faturamento e estratégias de venda"
        icon={GraduationCap}
        badge="17 cursos"
      >
        {/* Resumo de potencial */}
        <div className="p-3 rounded-xl bg-primary/5 border border-primary/20 space-y-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold text-foreground">Potencial de Faturamento</span>
          </div>
          <div className="grid grid-cols-3 gap-2">
            <div className="text-center p-2 rounded-lg bg-card border border-border">
              <div className="text-sm font-bold text-primary">R$ 18.370</div>
              <div className="text-[9px] text-muted-foreground">Catálogo completo</div>
            </div>
            <div className="text-center p-2 rounded-lg bg-card border border-border">
              <div className="text-sm font-bold text-primary">R$ 91.850</div>
              <div className="text-[9px] text-muted-foreground">5 alunos/curso</div>
            </div>
            <div className="text-center p-2 rounded-lg bg-card border border-border">
              <div className="text-sm font-bold text-primary">R$ 367.400</div>
              <div className="text-[9px] text-muted-foreground">20 alunos/curso</div>
            </div>
          </div>
        </div>

        {/* Tabela de preços por curso */}
        <div className="space-y-1.5">
          <p className="text-[11px] font-semibold text-foreground flex items-center gap-1.5">
            <DollarSign size={13} className="text-primary" />
            Sugestão de Preço por Curso
          </p>
          <div className="space-y-1">
            {[
              { nome: "Fitoterapia Clínica", horas: 140, preco: 1497, tier: "Premium" },
              { nome: "Fabricação de Óleos Essenciais", horas: 150, preco: 1497, tier: "Premium" },
              { nome: "Anatomia Humana", horas: 130, preco: 1297, tier: "Premium" },
              { nome: "Gastronomia Saudável", horas: 130, preco: 1197, tier: "Premium" },
              { nome: "Massagem Modeladora", horas: 128, preco: 1297, tier: "Premium" },
              { nome: "Seitai Japonês", horas: 125, preco: 1297, tier: "Premium" },
              { nome: "Bandagem Elástica", horas: 120, preco: 1097, tier: "Intermediário" },
              { nome: "Drenagem Linfática", horas: 116, preco: 1097, tier: "Intermediário" },
              { nome: "Difusor de Ambientes", horas: 105, preco: 897, tier: "Intermediário" },
              { nome: "Saboaria Artesanal", horas: 99, preco: 897, tier: "Intermediário" },
              { nome: "Velas Aromáticas", horas: 98, preco: 897, tier: "Intermediário" },
              { nome: "Vendas para Terapeutas", horas: 75, preco: 697, tier: "Essencial" },
              { nome: "Alta Perfumaria Natural", horas: 70, preco: 797, tier: "Intermediário" },
              { nome: "Yūgen FaceSPA", horas: 70, preco: 797, tier: "Intermediário" },
              { nome: "Head SPA Coreano", horas: 65, preco: 797, tier: "Intermediário" },
              { nome: "Aromaterapia dōTERRA", horas: 58, preco: 597, tier: "Essencial" },
              { nome: "Método Resinkra", horas: 24, preco: 497, tier: "Essencial" },
            ].map((curso) => (
              <div key={curso.nome} className="flex items-center gap-2 p-2 rounded-lg bg-muted/30 border border-border/40">
                <GraduationCap size={12} className="text-primary shrink-0" />
                <div className="flex-1 min-w-0">
                  <span className="text-[11px] font-semibold text-foreground block truncate">{curso.nome}</span>
                  <span className="text-[9px] text-muted-foreground">{curso.horas}h de conteúdo</span>
                </div>
                <Badge
                  variant={curso.tier === "Premium" ? "default" : "secondary"}
          ...
          <div className="p-2 rounded-lg bg-muted/50 border border-border/40 text-center">
            <span className="text-[10px] text-muted-foreground">Valor total do catálogo: </span>
            <span className="text-xs font-bold text-primary">R$ 18.370</span>
            <span className="text-[10px] text-muted-foreground"> · Ticket médio: </span>
            <span className="text-xs font-bold text-foreground">R$ 1.080</span>
          </div>
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <div className="space-y-1">
            {[
              { cenario: "Conservador", alunos: "5 alunos/mês", ticket: "R$ 797", fat: "R$ 3.985/mês", anual: "R$ 47.820/ano" },
              { cenario: "Moderado", alunos: "15 alunos/mês", ticket: "R$ 897", fat: "R$ 13.455/mês", anual: "R$ 161.460/ano" },
              { cenario: "Otimista", alunos: "30 alunos/mês", ticket: "R$ 1.097", fat: "R$ 32.910/mês", anual: "R$ 394.920/ano" },
              { cenario: "Agressivo", alunos: "50 alunos/mês", ticket: "R$ 1.097", fat: "R$ 54.850/mês", anual: "R$ 658.200/ano" },
            ].map((c) => (
              <div key={c.cenario} className="flex items-center gap-2 p-2.5 rounded-lg bg-muted/30 border border-border/40">
                <div className="flex-1 min-w-0">
                  <span className="text-[11px] font-bold text-foreground">{c.cenario}</span>
                  <span className="text-[9px] text-muted-foreground block">{c.alunos} · Ticket {c.ticket}</span>
                </div>
                <div className="text-right shrink-0">
                  <span className="text-xs font-bold text-primary block">{c.fat}</span>
                  <span className="text-[9px] text-muted-foreground">{c.anual}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="space-y-1.5">
          <p className="text-[11px] font-semibold text-foreground flex items-center gap-1.5">
            <DollarSign size={13} className="text-primary" />
            Modelos de Venda Sugeridos
          </p>
          <FeatureGrid items={[
            { icon: CreditCard, title: "Curso Individual", desc: "Venda avulsa com acesso vitalício + certificado" },
            { icon: Sparkles, title: "Combo Temático", desc: "3+ cursos relacionados com 20-30% desconto" },
            { icon: Users, title: "Assinatura Mensal", desc: "Acesso a todos os cursos por R$ 197-297/mês" },
            { icon: Target, title: "Formação Completa", desc: "Catálogo completo com mentoria: R$ 4.997-6.997" },
          ]} />
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <div className="space-y-1.5">
            {[
              {
                titulo: "🎯 Tráfego Pago (Google/Meta Ads)",
                acoes: [
                  "Campanhas de conversão com público lookalike de clientes da clínica",
                  "Retargeting para visitantes do site que não compraram",
                  "Anúncios em vídeo curto (Reels/Shorts) mostrando bastidores do curso",
                  "Budget sugerido: R$ 1.500-3.000/mês para ROI 3-5x",
                ],
              },
              {
                titulo: "📱 Conteúdo Orgânico (Resinkra AI)",
                acoes: [
                  "Usar Resinkra AI para gerar roteiros de Reels educativos",
                  "3-5 posts/semana com dicas do conteúdo dos cursos",
                  "Stories com enquetes e caixinhas de perguntas sobre temas dos cursos",
                  "Lives semanais com mini-aulas gratuitas como isca",
                ],
              },
              {
                titulo: "💬 WhatsApp Bot (Z-API)",
                acoes: [
                  "Sequência automática de nutrição: 7 msgs em 14 dias",
                  ...
                  "Envio de certificados de mini-curso gratuito como isca",
                ],
              },
              {
                titulo: "🤝 Programa de Afiliados / Parceiros",
                acoes: [
                  "Comissão de 20-30% para parceiros indicarem alunos",
                  "Material pronto (banners, textos, vídeos) para divulgação",
                  "Dashboard de acompanhamento de comissões no painel parceiro",
              ...
              },
              {
                ...
                acoes: [
                  ...
                  "Bônus exclusivos: checklist impresso + grupo VIP no WhatsApp",
                ],
              },
              {
                titulo: "📧 Email Marketing (Resend)",
                acoes: [
                  "Sequência de boas-vindas: 5 emails em 10 dias",
                  "Newsletter semanal com dicas e depoimentos de alunos",
                  "Campanhas de Black Friday, volta às aulas, datas comemorativas",
                  "Segmentação: leads frios → aquecidos → compradores → upsell",
                ],
              },
            ].map((estrategia) => (
              <div key={estrategia.titulo} className="p-2.5 rounded-lg bg-muted/30 border border-border/40 space-y-1.5">
                <span className="text-[11px] font-bold text-foreground">{estrategia.titulo}</span>
                <ul className="space-y-0.5">
                  {estrategia.acoes.map((acao, i) => (
        ...
        <div className="p-3 rounded-xl bg-primary/5 border border-primary/20 space-y-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold text-foreground">Funil de Vendas Sugerido</span>
          </div>
          <div className="space-y-1">
            {[
              { step: "1", label: "Atração", desc: "Reels/Ads → Landing page do curso", color: "text-blue-500" },
              { step: "2", label: "Captura", desc: "Mini-curso grátis → Coleta email/WhatsApp", color: "text-yellow-500" },
              { step: "3", label: "Nutrição", desc: "Sequência WhatsApp + Email (7-14 dias)", color: "text-orange-500" },
              { step: "4", label: "Conversão", desc: "Oferta com urgência + bônus exclusivos", color: "text-green-500" },
              { step: "5", label: "Upsell", desc: "Combo/assinatura para quem já comprou", color: "text-primary" },
            ].map((s) => (
              <div key={s.step} className="flex items-center gap-2.5 p-1.5">
                <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold bg-primary/10 ${s.color}`}>
                  {s.step}
                </div>
