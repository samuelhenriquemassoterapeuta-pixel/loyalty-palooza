    ...
    >
      ...
      {stats && (
        <div className="grid grid-cols-3 gap-2">
          ...
          <div className="p-2.5 rounded-xl bg-muted/40 border border-border/50 text-center">
            <p className="text-[10px] text-muted-foreground uppercase tracking-wide">Variação</p>
            <p
              className={`text-sm font-bold ${
                stats.diff < 0 ? "text-highlight" : stats.diff > 0 ? "text-destructive" : "text-muted-foreground"
              }`}
            >
              {stats.diff > 0 ? "+" : ""}
              {stats.diff}
